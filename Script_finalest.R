...
#################### PCA ######################
train <- train[!rowSums(is.na(train)), -1]

train[,13:31] <- train[,13:31]*1
meanValues <- colMeans(train[,1:31])
train <- sweep(train[,1:31],2, meanValues, "-")


components <- prcomp(as.matrix(train[,-1]), retx = TRUE, na.action=na.omit, center = FALSE, scale = FALSE)
eigenMatrix <- t(components$rotation) #store the featureVector matrix 

plot(summary(components)$importance[3,], type="l", ylab="%variance explained", xlab="nth component (decreasing order)")
abline(h=0.99,col="red")
# to capture 99% of the variance, we need the first 4 components
abline(v=4,col="red",lty=3)


pc.use <- 4
trunc1 <- components$x[,1:pc.use] 
modFeatures <- trunc1[!rowSums(is.na(trunc1)), ]
modFeatures <- cbind(modFeatures, num_reviews = as.integer(train[,1]))

index_pca_test = sample(1:nrow(modFeatures), size=0.7*nrow(modFeatures))
train_set=modFeatures[index_pca_test,]
test_set=modFeatures[-index_pca_test,]


##### Build the model #####
model_t2 <- svm(num_reviews~. , train_set, kernel = "linear", cross = 10)
m <- predict(model_t2, newdata = train_set)

x <- error_pc(model_t2$residuals, train_set[,"num_reviews"])
x[mapply(is.infinite, x)] <- NA
mean(x, na.rm = TRUE) # error % is 5.429086

y <- error_ratio(train_set[,"num_reviews"], m)
mean(y) # error ratio is 0.0531761

### Test Set ###
tunedModelY <- predict(model_t2, newdata = test_set)

x <- error_pc(test_set[,"num_reviews"]-tunedModelY, test_set[,"num_reviews"])
x[mapply(is.infinite, x)] <- NA
mean(x, na.rm = TRUE) # error % is 5.590179

y <- error_ratio(test_set[,"num_reviews"], tunedModelY)
mean(y) # error ratio is 0.03336644
