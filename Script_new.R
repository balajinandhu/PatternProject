...
test <-getTimeVectorsBeforeDt("2013-09-01")
test <- test[which(test$business_id %in% tbIDs),]

########################### TRAINING SET ###########################
modFeatures <- train[!rowSums(is.na(train)), -1]
...
################ TESTING SET #################
modFeatures1 <- test[!rowSums(is.na(test)), -1]
